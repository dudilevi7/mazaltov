...
const AppSidebar = () => {
  ...
  return (
    <div
      ...
      dir={isRtl ? 'rtl' : 'ltr'}>
      <div
        className={`flex flex-col h-full ${isSidebarOpen ? 'py-4 pe-3 animate-fade-in-0.5' : 'py-4 px-0 items-center'}`}>
            ...
            return (
              <Tooltip
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-md text-gray-700 hover:bg-linear-to-b hover:from-blue-50 hover:to-blue-100 transition-colors ${
                    isSidebarOpen
                      ? `w-full px-3 py-2 text-sm font-medium ${isRtl ? 'flex flex-row' : 'flex justify-start'}`
                      : 'w-10 h-10 justify-center rounded-s-none'
                  } ${isActive ? 'bg-linear-to-b from-blue-50 to-blue-100 shadow-xs' : 'text-gray-500'} cursor-pointer`}>
                  <FontAwesomeIcon
                    icon={item.icon}
